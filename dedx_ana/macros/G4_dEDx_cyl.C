...
void Svtx_Reco(int verbosity = 0)
{
  ...
  /*
    ...
    se->registerSubsystem( tpcclusterizer ); // uncomment it if one wants to use tpc clusters in hough transform just like default version
  */
  
  PHG4TPCClusterizer* tpcclusterizer = new PHG4TPCClusterizer();
  //tpcclusterizer->Verbosity(2001);
  tpcclusterizer->Verbosity(2);
  tpcclusterizer->setEnergyCut(20/*adc*/);
  tpcclusterizer->setRangeLayers(n_ib_layer+n_intt_layer,Max_si_layer);
  tpcclusterizer->setFitWindowSigmas(diffusion,diffusion);
