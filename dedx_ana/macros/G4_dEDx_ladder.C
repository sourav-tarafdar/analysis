...
bool use_primary_vertex = false /*true*/;   // include primary vertex in track fit if true - writes a second set of tracks to node tree
...
void Svtx_Cells(int verbosity = 0)
{
  ...
  // Haiwang's memory reducer for 40 layers
...
void Svtx_Reco(int verbosity = 0)
{
  ...
  if(n_gas_layer == 40)
    {
      // new from Carlos for modified clustering, have to build g4hough and g4detectors
      ...
      //tpcclusterize9->setFitWindowMax(9/*rphibins*/,7/*zbins*/); 
      tpcclusterizer->setFitWindowMax(4/*rphibins*/,3/*zbins*/);  // reduced from Carlos values (above)
      //tpcclusterizer->setFitEnergyThreshold( 0.1 /*fraction*/ );
      tpcclusterizer->setFitEnergyThreshold( 0.05 /*fraction*/ ); // for double cell size
      //tpcclusterizer->setFitEnergyThreshold( 500 /*absolute*/ );
    }
  else
    {
      // 60 layer tune  
...
void dEdX_Eval(std::string outputfile, int verbosity = 0)
{
  //---------------
  // Load libraries
  //---------------
  ...
  dEDxEval* eval = new dEDxEval("DEDXEVAL", outputfile.c_str());
