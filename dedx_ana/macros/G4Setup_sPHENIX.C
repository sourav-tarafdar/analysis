  ...
  if (do_svtx)
    {
      ...
      gROOT->LoadMacro("G4_dEDx_ladder.C");      // cylinder cell maps IB + intermediate tracker + tpc
      //gROOT->LoadMacro("G4_Svtx_maps_ladders+intt_ladders+tpc_40layer_KalmanPatRec_Tony.C");
      SvtxInit();
...
int G4Setup(const int absorberactive = 0,
  ...
  // g4Reco->save_DST_geometry(true); // Need for truth pattern reco from Haiwang
...
void ShowerCompress(int verbosity = 0) {
  ...
  //gSystem->Load("/sphenix/user/isibf5y/install_g4det_sbu/lib/libg4eval.so");
